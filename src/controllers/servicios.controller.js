...
class ServiciosController {
    ...
    async crear(data) {
        try {
            ...
            const [resultado] = await db.query(
                'INSERT INTO Servicios (servNombre, servDescripcion, servCosto, servImagen) VALUES (?, ?, ?, ?)',
                [data.servNombre, data.servDescripcion, data.servCosto, servImagen]
            );
    ...
    async actualizar(id, data) {
        try {
            ...
            const servImagen = data.servImagen ? Buffer.from(data.servImagen, 'base64') : null;
            const [resultado] = await db.query(
                'UPDATE Servicios SET servNombre = ?, servDescripcion = ?, servCosto = ?, servImagen = ? WHERE idServicios = ?',
                [data.servNombre, data.servDescripcion, data.servCosto, servImagen, id]
            );
